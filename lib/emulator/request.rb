...
module OssEmulator
  class Request
    ...
    DELETE_BUCKET_REFERER = "DELETE_BUCKET_REFERER"
    ...
    def parse_put()
      if @path == "/"
        ...
        if @is_path_style
          ...
          if elems.size == 1
            ...
            if @path.end_with?('/') && !@object.end_with?('/')
              @object += '/'
            end
          end
        else
      ...
      if copy_source && copy_source.size == 1
        ...
        @src_object = CGI.unescape(src_elems[1 + root_offset,src_elems.size].join("/"))
        if @cmd != Request::PUT_UPLOAD_PART && @cmd != Request::PUT_UPLOAD_PART_COPY
          @cmd = Request::PUT_COPY_OBJECT
        end
      end
    end
    ...
    def parse_get()
      ...
      if @path == "/" && @is_path_style
        ...
        if @is_path_style
          ...
          elems = @path.split("/").reject(&:empty?)
        end

        bucket_only = @is_path_style ? elems.size < 2 : (elems.empty? || @bucket.to_s.empty?)
        if bucket_only  # bucket only
          if !@is_path_style && @bucket.to_s.empty?
            @cmd = Request::LIST_BUCKETS
            return
          end
          if @is_path_style && elems.size == 1
            candidate_bucket = elems[0]
            bucket_meta = File.join(Config.store, candidate_bucket, Store::BUCKET_METADATA)
            if !File.exist?(bucket_meta)
              matched_bucket = nil
              Dir[File.join(Config.store, "*")].each do |bucket_dir|
                bucket_name = File.basename(bucket_dir)
                next unless File.exist?(File.join(bucket_dir, Store::BUCKET_METADATA))
                if File.exist?(File.join(bucket_dir, elems[0], Store::OBJECT_METADATA)) ||
                   File.exist?(File.join(bucket_dir, "#{elems[0]}/", Store::OBJECT_METADATA))
              ...
              if matched_bucket
                ...
                @object = elems[0]
                @cmd = (@method=="HEAD") ? Request::HEAD_OBJECT : Request::GET_OBJECT
                return
              end
            end
          end
          ...
          @object = @is_path_style ? elems[1,elems.size].join('/') : elems.join('/')
          if @is_path_style && elems.size >= 2
            bucket_meta = File.join(Config.store, @bucket, Store::BUCKET_METADATA)
            if !File.exist?(bucket_meta)
              full_object = elems.join('/')
              if @path.end_with?('/') && !full_object.end_with?('/')
                full_object += '/'
              end
              matched_bucket = nil
              Dir[File.join(Config.store, "*")].each do |bucket_dir|
                bucket_name = File.basename(bucket_dir)
                next unless File.exist?(File.join(bucket_dir, Store::BUCKET_METADATA))
                if File.exist?(File.join(bucket_dir, full_object, Store::OBJECT_METADATA)) ||
                   File.exist?(File.join(bucket_dir, "#{full_object}/", Store::OBJECT_METADATA))
                  matched_bucket = bucket_name
                  break
                end
              end
      ...
      if @path == "/" && @is_path_style
        ...
        if elems.size == 0
          ...
          if @request.request_line =~ /\?logging/
            ...
            @cmd = Request::DELETE_BUCKET_REFERER
          elsif @request.request_line =~ /\?lifecycle/
