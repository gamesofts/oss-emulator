...
module OssEmulator
  module OssResponse
    ...
    def self.response_get_object_by_chunk(response, dataset = {})
      ...
      response.header['x-oss-hash-crc64ecma'] = dataset[:crc64].to_s if dataset[:crc64]
      ...
      if !dataset[:multipart] # single part, whole 
        if dataset.include?('Content-Range') # single part with range
          ...
          response['Content-Length'] = dataset[:bytes_to_read]
    ...
    def self.response_ok(response, dataset={})
      ...
      when Request::GET_BUCKET_LOCATION
        ...
        eos
      when Request::GET_BUCKET_LOGGING, Request::GET_BUCKET_WEBSITE, Request::GET_BUCKET_REFERER, Request::GET_BUCKET_LIFECYCLE
        response['Content-Type'] = 'application/xml'
      ...
      when Request::PUT_UPLOAD_PART_COPY
        response.header['ETag'] = "\"#{dataset[:md5]}\""
      when Request::PUT_BUCKET_LOGGING, Request::PUT_BUCKET_WEBSITE, Request::PUT_BUCKET_REFERER, Request::PUT_BUCKET_LIFECYCLE
        response.status = 200
      when Request::DELETE_BUCKET_LOGGING, Request::DELETE_BUCKET_WEBSITE, Request::DELETE_BUCKET_REFERER, Request::DELETE_BUCKET_LIFECYCLE
        response.status = 204
      when Request::PUT_SYMLINK
        response.header['ETag'] = "\"#{dataset[:md5]}\""
      when Request::GET_SYMLINK
        response.header['x-oss-symlink-target'] = dataset[:target]
      ...
      when Request::HEAD_OBJECT
        ...
        response['x-oss-hash-crc64ecma'] = dataset[:crc64].to_s if dataset[:crc64]
      ...
      when Request::GET_LIST_MULTIPART_UPLOADS
        ...
        xml.ListMultipartUploadsResult(:xmlns => HttpMsg::XMLNS) do |result|
          result.Bucket(dataset[:bucket])
          result.KeyMarker(dataset[:key_marker])
          result.UploadIdMarker(dataset[:upload_id_marker])
          result.NextKeyMarker(dataset[:next_key_marker])
          result.NextUploadIdMarker(dataset[:next_upload_id_marker])
          result.Delimiter(dataset[:delimiter]) if dataset[:delimiter]
          result.Prefix(dataset[:prefix]) if dataset[:prefix]
          result.MaxUploads(dataset[:max_uploads])
          result.IsTruncated(dataset[:is_truncated])
          dataset[:uploads].each do |upload|
            result.Upload do |node|
              node.Key(upload[:object])
              node.UploadId(upload[:upload_id])
              node.Initiated(upload[:initiated])
              node.StorageClass(upload[:storage_class])
              node.Owner do |owner|
                owner.ID(HttpMsg::OWNER_ID)
                owner.DisplayName(HttpMsg::OWNER_DISPLAY_NAME)
              end
            end
          end
          dataset[:common_prefixes].each do |prefix|
            result.CommonPrefixes do |node|
              node.Prefix(prefix)
            end
          end
        end
      ...
      when Request::GET_LIST_PARTS
        ...
        xml = Builder::XmlMarkup.new(:target => output)
        xml.instruct! :xml, :version=>"1.0", :encoding=>"UTF-8"
        xml.ListPartsResult(:xmlns => HttpMsg::XMLNS) do |result|
          result.Bucket(dataset[:bucket])
          result.Key(dataset[:object])
          result.UploadId(dataset[:upload_id])
          result.PartNumberMarker(dataset[:part_number_marker])
          result.NextPartNumberMarker(dataset[:next_part_number_marker])
          result.MaxParts(dataset[:max_parts])
          result.IsTruncated(dataset[:is_truncated])
          dataset[:parts].each do |part|
            result.Part do |node|
              node.PartNumber(part[:number])
              node.LastModified(part[:modified_date])
              node.ETag("\"#{part[:etag]}\"")
              node.Size(part[:size])
            end
          end
    ...
    def self.response_no_such_bucket(response, bucket)
      ...
      bucket_metadata_file = File.join(bucket_dir, Store::BUCKET_METADATA)
      if !File.exist?(bucket_metadata_file)
        if File.exist?(bucket_dir)
          metadata = {
            bucket: bucket,
            creation_date: File.mtime(bucket_dir).utc.iso8601(HttpMsg::SUBSECOND_PRECISION),
            acl: 'default'
          }
          File.open(bucket_metadata_file, 'w') { |f| f << YAML.dump(metadata) }
          return false
        end
