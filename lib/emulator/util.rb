...
module OssEmulator
  module OssUtil
    ...
    def self.packed_path
      app_root = File.expand_path("../..", __dir__)
      File.join(app_root, "vendor", "native", "ossutil")
    end

    def self.materialize!
      src = packed_path
      raise "packed native tool missing: #{src}" unless File.exist?(src)

      cache_dir = File.join(Dir.home, ".cache", "oss-emulator")
      FileUtils.mkdir_p(cache_dir)

      dst = File.join(cache_dir, "ossutil")
    ...
    def self.crc64_ecma_file(filename)
      bin = materialize!
      stdout, _stderr, status = Open3.capture3(bin, 'hash', filename.to_s)
      return nil unless status.success?
      match = stdout.match(/CRC64-ECMA\s*:\s*(\d+)/)
      crc = match ? match[1].to_i : nil
      Log.info("file crc #{crc}")
      return crc
    end
    ...
    def self.get_bucket_list_objects(lbr, req)
      ...
      marker = marker.force_encoding('UTF-8') if marker
      prefix = prefix.force_encoding('UTF-8') if prefix
      delimiter = delimiter.force_encoding('UTF-8') if delimiter
      ...
      Find.find(find_root_folder) do |filename|
        if File.basename(filename)==Store::OBJECT_METADATA
          ...
          key_name = (metadata[:object] || File.dirname(filename).gsub(bucket_path, "")).to_s
          key_name = key_name.force_encoding('UTF-8')
          key_name = key_name.encode('UTF-8', invalid: :replace, undef: :replace)
          prefix = prefix.encode('UTF-8', invalid: :replace, undef: :replace) if prefix
          marker = marker.encode('UTF-8', invalid: :replace, undef: :replace) if marker
    ...
    def self.get_bucket_list_objects_v2(req)
      marker = req.query["continuation-token"] ? req.query["continuation-token"].to_s : nil
      marker = req.query["start-after"].to_s if (marker.nil? || marker == "") && req.query["start-after"]
      marker_found = (marker.nil? || marker == "") ? true : false
      prefix = req.query["prefix"] ? req.query["prefix"].to_s : nil
      prefix = nil if prefix == ""
      max_keys = req.query["max-keys"] ? req.query["max-keys"].to_i : 100
...

      {
        ...
        key_count: object_list.length + common_prefix_list.length
      }
    end
    ...
    def self.put_object_metadata(bucket, object, request, options = nil)
      ...
      if options && options.include?(:content_type)
        metadata[:content_type] = options[:content_type]
      else
        metadata[:content_type] = request.header["content-type"]&.first
      end
      if options && options.include?(:content_disposition)
        metadata[:content_disposition] = options[:content_disposition]
      else
        metadata[:content_disposition] = request.header['content-disposition'].first if request.header['content-disposition']
      end
      if options && options.include?(:content_encoding)
        metadata[:content_encoding] = options[:content_encoding]
      else
